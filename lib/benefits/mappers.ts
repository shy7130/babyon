...
export function toBenefitRow(row: Record<string, any>): BenefitRow {
  return {
    ...
    hasDirectApplyLink: row.has_direct_apply_link,
    amountManwon: row.amount_manwon,
    status: row.status,
...
export function toInsertRow(record: BenefitRecord, status: BenefitStatus): Record<string, any> {
  return {
    ...
    has_direct_apply_link: record.hasDirectApplyLink,
    amount_manwon: record.amountManwon,
    status,
